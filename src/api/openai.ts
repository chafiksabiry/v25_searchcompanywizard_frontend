...
1. COMPANY_INTRO: Write a compelling introduction for a "Why Partner With Us?" page (3-4 lines maximum, modern and dynamic tone)

2. UNIQUENESS_CATEGORIES: Generate 4-6 categories that highlight why someone should partner with this company. Each category should include:
- title: A compelling category name
- description: Brief description of the category  
- score: A number from 1-5 representing the strength
- details: An array of 3-5 specific benefits or features
...
Return the response as a valid JSON object with this exact structure:
{
  "companyIntro": "string (the introduction text)",
  "categories": [
    {
      "title": "string",
      "icon": "iconName",
...
Make everything relevant to the company's industry and strengths. Focus on what makes this company unique and attractive to potential partners.`;
  ...
  try {
    ...
    return {
      companyIntro: parsedResponse.companyIntro,
      categories: categories,
    };
  } catch (error) {
    console.error("OpenAI API Error:", error);
    throw new Error("Failed to generate uniqueness content");
  }
}
